...
const getAnswersCount = `-- name: GetAnswersCount :one
SELECT COUNT(*)
FROM "answers"
`

func (q *Queries) GetAnswersCount(ctx context.Context) (int64, error) {
	row := q.db.QueryRowContext(ctx, getAnswersCount)
	var count int64
	err := row.Scan(&count)
	return count, err
}
